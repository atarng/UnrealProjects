...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
    ...
    CollectionSphere->SetupAttachment(RootComponent); //CollectionSphere->AttachTo(RootComponent);
...
void ABatteryCollectorCharacter::CollectPickups() {
    ...
    for (int iCollected = 0; iCollected < CollectedActors.Num(); ++iCollected) {
        ...
        // If Successful, and the pickup is valid and active, 
        if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive()) {

            // call the pickup's WasCollected function
            TestPickup->WasCollected();

            // Check to see if batter
...
// Called everytime power is increased or decreased
